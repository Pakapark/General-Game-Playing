...
public final class Scheduler implements Observer
{
	...
	private Map<String, WeakReference<IIGameServer>> iigameServers;

	private boolean isIIScheduler = true;

	public Scheduler(JTabbedPane matchesTabbedPane, SchedulingPanel schedulingPanel, LeaderboardPanel leaderboardPanel, boolean isIIScheduler) {
		...
		iigameServers = new HashMap<String, WeakReference<IIGameServer>>();
		this.isIIScheduler = isIIScheduler;
	}
	...
	public boolean isIIScheduler() {
		return isIIScheduler;
	}
	...
	private synchronized void doSchedule(PendingMatch spec) {
		if (isIIScheduler) doIISchedule(spec);
		else doNormalSchedule(spec);
	}

	private synchronized void doNormalSchedule(PendingMatch spec) {
		try {
	...
	private synchronized void doIISchedule(PendingMatch spec) {
		try {
			IIMatch match = new IIMatch(spec.matchID, spec.previewClock, spec.startClock, spec.playClock, spec.theGame, "");
			...
			IIGameServer gameServer = new IIGameServer(match, hosts, ports);
			if (spec.shouldDetail) {
				gameServer.addObserver(errorPanel);
				gameServer.addObserver(historyPanel);
				gameServer.addObserver(visualizationPanel);
			...
			iigameServers.put(spec.matchID, new WeakReference<IIGameServer>(gameServer));
			schedulingQueue.remove(spec);
		} catch (Exception e) {
			e.printStackTrace();
		}
