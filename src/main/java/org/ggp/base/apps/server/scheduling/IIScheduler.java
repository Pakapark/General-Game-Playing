...
public final class IIScheduler implements Observer
{
	public EncodedKeyPair signingKeys;

	private JTabbedPane matchesTabbedPane;
	private IISchedulingPanel schedulingPanel;
	private LeaderboardPanel leaderboardPanel;

	private final List<PendingMatch> schedulingQueue;
	private final Set<String> activePlayers;

	private Map<String, WeakReference<IIGameServer>> gameServers;

	public IIScheduler(JTabbedPane matchesTabbedPane, IISchedulingPanel schedulingPanel, LeaderboardPanel leaderboardPanel) {
		...
		gameServers = new HashMap<String, WeakReference<IIGameServer>>();
	}
	...
	private synchronized void doSchedule(PendingMatch spec) {
		try {
			...
			gameServers.put(spec.matchID, new WeakReference<IIGameServer>(gameServer));
	...
	public void observe(Event genericEvent) {
		...
		IIMatch match = (IIMatch) event.getMatch();
