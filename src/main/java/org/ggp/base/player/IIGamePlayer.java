...
public final class IIGamePlayer extends Thread implements Subject
{
    private final int port;
    private final IIGamer gamer;
    private ServerSocket listener;
    private final List<Observer> observers;

    // consider changing this so that it takes an IIGamer as an argument
    // rather than just a Gamer
    public IIGamePlayer(int port, Gamer gamer) throws IOException
    {
	...
	public static void main(String[] args)
	{
		...
		try {
			IIGamePlayer player = new IIGamePlayer(Integer.valueOf(args[0]), new RandomGamer());
			player.run();
